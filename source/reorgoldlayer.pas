...
function make_reorg_old_layer(const batch, w, h, c, stride: longint; const reverse: boolean):TReOrgOldLayer;
procedure resize_reorg_old_layer(var l: TReOrgOldLayer; const w, h: longint);
procedure forward_reorg_old_layer(var l: TReOrgOldLayer; const state: PNetworkState);
procedure backward_reorg_old_layer(var l: TReOrgOldLayer; const state: PNetworkState);

{$ifdef GPU}
procedure forward_reorg_old_layer_gpu(l: layer; state: network_state);
procedure backward_reorg_old_layer_gpu(l: layer; state: network_state);
{$endif}
...
var
    output_size: longint;
begin
    result := Default(TReOrgOldLayer);
    result.&type := ltREORG_OLD;
    result.batch := batch;
    ...
    writeln(ErrOutput, format('reorg_old              /%2d  %4d x%4d x%4d   ->  %4d x%4d x%4d', [stride, w, h, c, result.out_w, result.out_h, result.out_c]));
    ...
    result.forward := forward_reorg_old_layer;
    result.backward := backward_reorg_old_layer;
{$ifdef GPU}
    result.forward_gpu := forward_reorg_old_layer_gpu;
    result.backward_gpu := backward_reorg_old_layer_gpu;
...
begin
    ...
    //if l.reverse then
        reorg_cpu(state.input, l.w, l.h, l.c, l.batch, l.stride, l.reverse, l.output) ;
...
begin
    //if l.reverse then
        reorg_cpu(l.delta, l.w, l.h, l.c, l.batch, l.stride, l.Reverse, state.delta)
    //else
